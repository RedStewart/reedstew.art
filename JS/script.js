...
for (let i = 0; i < cornerButtons.length; i++) {
  cornerButtons[i].addEventListener('click', function() {
    buttonClick(this);
  });
}
...
function fadeInfo(cornClick) {
  ...
  if (cornClick == 'About') {
    ...
    deskSkillsHoverListener();
    if (mobileDevice) mainDiv.style.width = '62%';
...
const setMobileMargin = () => {
  document.getElementById('cornContentContain').style.width = '95%';
};
